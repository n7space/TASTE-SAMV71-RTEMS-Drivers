...
void BitStream_AttachBuffer2(BitStream *pBitStrm, unsigned char *buf,
			     long count, void *pushDataPrm, void *fetchDataPrm)
{
...
void BitStream_AppendNBitZero(BitStream *pBitStrm, int nbits)
{
	...
	if (pBitStrm->currentByte + totalBytes <= pBitStrm->count) {
		...
		int extraBytes =
			pBitStrm->currentByte + totalBytes - pBitStrm->count;
...
{
	if (v) {
		pBitStrm->buf[pBitStrm->currentByte] |=
			masks[pBitStrm->currentBit];
	} else {
...
flag BitStream_ReadBit(BitStream *pBitStrm, flag *v)
{
	*v = pBitStrm->buf[pBitStrm->currentByte] & masks[pBitStrm->currentBit];
	...
	return pBitStrm->currentByte * 8 + pBitStrm->currentBit <=
	       pBitStrm->count * 8;
}
...
/* nbits 1..7*/
void BitStream_AppendPartialByte(BitStream *pBitStrm, byte v, byte nbits,
				 flag negate)
{
	int cb = pBitStrm->currentBit;
	...
	if (totalBits <= 8) {
		...
		pBitStrm->buf[pBitStrm->currentByte++] |=
			(byte)(v >> totalBitsForNextByte);
		bitstream_push_data_if_required(pBitStrm);
		byte mask = (byte)~masksb[8 - totalBitsForNextByte];
		pBitStrm->buf[pBitStrm->currentByte] &= mask;
		pBitStrm->buf[pBitStrm->currentByte] |=
			(byte)(v << (8 - totalBitsForNextByte));
...
flag BitStream_ReadPartialByte(BitStream *pBitStrm, byte *v, byte nbits)
{
	...
	if (totalBits <= 8) {
		*v = (byte)((pBitStrm->buf[pBitStrm->currentByte] >>
			     (8 - totalBits)) &
			    masksb[nbits]);
		pBitStrm->currentBit += nbits;
		if (pBitStrm->currentBit == 8) {
						    ...
						    flag negate)
{
	...
	if (pbits) {
		cc -= pbits;
		BitStream_AppendPartialByte(pBitStrm, (byte)(v >> cc),
					    (byte)pbits, negate);
	}
...
#if WORD_SIZE == 8
	if (v < 0x100000000LL)
		BitStream_EncodeNonNegativeInteger32Neg(pBitStrm,
							(asn1SccUint32)v, 0);
	else {
...
flag BitStream_DecodeNonNegativeInteger(BitStream *pBitStrm, asn1SccUint *v,
					int nBits)
{
#if WORD_SIZE == 8
	asn1SccUint32 hi = 0;
	...
	if (nBits <= 32) {
		ret = BitStream_DecodeNonNegativeInteger32Neg(pBitStrm, &lo,
							      nBits);
		*v = lo;
		return ret;
	}

	ret = BitStream_DecodeNonNegativeInteger32Neg(pBitStrm, &hi, 32) &&
	      BitStream_DecodeNonNegativeInteger32Neg(pBitStrm, &lo,
						      nBits - 32);
...
void BitStream_EncodeConstraintWholeNumber(BitStream *pBitStrm, asn1SccSint v,
					   asn1SccSint min, asn1SccSint max)
{
	int nRangeBits;
	int nBits;
	asn1SccUint range;
...
void BitStream_EncodeConstraintPosWholeNumber(BitStream *pBitStrm,
					      asn1SccUint v, asn1SccUint min,
					      asn1SccUint max)
{
	int nRangeBits;
	int nBits;
...
flag BitStream_DecodeConstraintWholeNumber(BitStream *pBitStrm, asn1SccSint *v,
					   asn1SccSint min, asn1SccSint max)
{
	asn1SccUint uv;
	int nRangeBits;
...
flag BitStream_DecodeConstraintPosWholeNumber(BitStream *pBitStrm,
					      asn1SccUint *v, asn1SccUint min,
					      asn1SccUint max)
{
	asn1SccUint uv;
...
flag DecodeRealAsBinaryEncoding(BitStream *pBitStrm, int length, byte header,
				asn1Real *v);
